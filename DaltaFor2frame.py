...
# =============================================
# حساب متوسط frame_time_delta لكل قياسين (كل 2 فريم)
# =============================================

# ترتيب البيانات حسب IP والوقت
df = df.sort_values(['ip_src', 'time_epoch'])

# حساب المتوسط المتحرك لكل 2 فريم
df['frame_time_delta_mean_2frames'] = df.groupby('ip_src')['frame_time_delta'].rolling(
    window=2, min_periods=1
).mean().reset_index(level=0, drop=True)

# حفظ النتائج النهائية
df.to_csv('11sssensor_data_features_with_2frame_mean.csv', index=False)

# عرض النتائج
print(f"\nإحصائيات frame_time_delta:")
print(f"  - المتوسط العام: {df['frame_time_delta'].mean():.6f} ثانية")
print(f"  - المتوسط لكل 2 فريم: {df['frame_time_delta_mean_2frames'].mean():.6f} ثانية")

print(f"\nعينة من البيانات مع المتوسط المحسوب:")
sample_columns = [
    'ip_src', 'time_epoch', 'frame_time_delta', 'frame_time_delta_mean_2frames'
]
print(df[sample_columns].head(10))

print(f"\nتم حفظ البيانات مع المتوسط في: 11sssensor_data_features_with_2frame_mean.csv")